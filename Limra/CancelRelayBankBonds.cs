...
namespace Limra
{
    public partial class CancelRelayBankBonds : Form
    {
        ...
        private void loadDataIntoGridView1()
        {
            ...
            {
                con.Open();
                cmd.CommandText = "SELECT DISTINCT docID, enterDate FROM RelayVoucherOfExchange WHERE boxNum=@boxNum AND docKind=@docKind";
                cmd.Connection = con;
        ...
        private void loadDataIntoGridView2()
        {
            if (textBoxBoxNumber.Text != "")
            {
                ...
                {
                    con.Open();
                    cmd.CommandText = "SELECT DISTINCT docID, enterDate FROM RelayVoucherOfExchange WHERE " +
        ...
        private void loadDataIntoGridViewBoxes()
        {
            if (textBoxBoxNumber.Text != "")
            {
                ...
                {
                    con.Open();
                    cmd.CommandText = "SELECT DISTINCT boxNum, boxName FROM RelayVoucherOfExchange WHERE docKind=@docKind";
                    cmd.Connection = con;
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("@docKind", "شيك");
        ...
        {  //////////////SAVE//////////////////////////
            ...
            {
                insertIntoNonRelayDatabase();
                deleteFromRelayDataBase();
            }
        }

        private void deleteFromRelayDataBase()
        {
            con.Open();
            for (int i = 0; i < dataGridView2.Rows.Count; i++)
            {
                cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "DELETE FROM RelayVoucherOfExchange WHERE docId=@docId ";
                cmd.Parameters.Clear();
        ...
        private void insertIntoNonRelayDatabase()
        {
            try
            {
                con.Open();
                for (int i = 0; i < dataGridView2.Rows.Count; i++)
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO VoucherOfExchange" +
                    ...
                    "accNum,accName,notes,cost,enterDate,refrencer FROM RelayVoucherOfExchange where docID=@docID");
                    cmd.Connection = con;
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("@docID", dataGridView2.Rows[i].Cells[0].Value);
                    cmd.ExecuteNonQuery();
