...
namespace Limra
{
    public partial class VoucherOfExchange : Form
    {
        ...
        public VoucherOfExchange()
        {
            InitializeComponent();
        }
        //FormLoad
        private void VoucherOfExchange_Load(object sender, EventArgs e)
        {
            ...
            if (textBoxDocNum.Text != "" && textBoxNum.Text != "")
            {
                ...
                try
                {
                    ...
                    cmd.CommandText = "SELECT Id, docKind, enterDate, boxDescription, accNum," +
                        " accName, notes, cost, refrencer FROM VoucherOfExchange" +
        ...
        private void insertIntoVocher()
        {
            try
            {
                ...
                con.Open();
                cmd.CommandText = "INSERT INTO VoucherOfExchange" +
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count != 0)
            {
                ...
                string boxNumber = getVoucherOfExchange_boxNumber(value);
        ...
        {
            con.Open();
            string sqlquery = "DELETE FROM VoucherOfExchange WHERE id = @id";
            SqlCommand command = new SqlCommand(sqlquery, con);
            command.Parameters.Clear();
        ...
        {
            con.Open();
            SqlCommand check_Box_Num = new SqlCommand("SELECT * FROM VoucherOfExchange WHERE boxNum=@boxNum", con);
        ...
        {
            con.Open();
            string sql = "Truncate Table VoucherOfExchange";
        ...
        private string getVoucherOfExchange_docID(string num)
        { // get docID
            string results;
            con.Open();
            string sqlquery = "SELECT docID FROM VoucherOfExchange WHERE accNum=@accNum";
            SqlCommand command = new SqlCommand(sqlquery, con);
            command.Parameters.Clear();
        ...
        private string getVoucherOfExchange_boxNumber(string num)
        { // get boxNumber
            string results;
            con.Open();
            string sqlquery = "SELECT boxNum FROM VoucherOfExchange WHERE accNum=@accNum";
            SqlCommand command = new SqlCommand(sqlquery, con);
            command.Parameters.Clear();
            command.Parameters.AddWithValue("@accNum", num);
            results = command.ExecuteScalar().ToString();
        ...
        private string getVoucherOfExchange_boxName(string num)
        { // get boxName
            string results;
            con.Open();
            string sqlquery = "SELECT boxName FROM VoucherOfExchange WHERE accNum=@accNum";
            SqlCommand command = new SqlCommand(sqlquery, con);
            command.Parameters.Clear();
            command.Parameters.AddWithValue("@accNum", num);
            results = command.ExecuteScalar().ToString();
        ...
        private string getVoucherOfExchange_boxCost(string num)
        { // get boxName
            string results;
            con.Open();
            string sqlquery = "SELECT boxCost FROM VoucherOfExchange WHERE accNum=@accNum";
            SqlCommand command = new SqlCommand(sqlquery, con);
            command.Parameters.Clear();
            command.Parameters.AddWithValue("@accNum", num);
            results = command.ExecuteScalar().ToString();
        ...
        private void dataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                textBoxDocNum.Text = getVoucherOfExchange_docID(accNumber); //docID
                textBoxNum.Text = getVoucherOfExchange_boxNumber(accNumber);
                textBoxName.Text = getVoucherOfExchange_boxName(accNumber);
                textBoxCost.Text = getVoucherOfExchange_boxCost(accNumber); //boxCost
       ...
       private void needUpdate(string txtAccNum, string txtAccName)
        ...
        private void VoucherOfExchange_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (allCostsEqual())
            {
            ...
            {
                e.Cancel = true;
                MessageBox.Show("المبلغ غير متساوٍ");
            }
        }
