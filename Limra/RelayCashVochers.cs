...
namespace Limra
{
    public partial class RelayCashVochers : Form
    {
        ...
        private void loadDataIntoGridView1()
        {
            ...
            {
                con.Open();
                cmd.CommandText = "SELECT DISTINCT docID, enterDate FROM VoucherOfExchange WHERE boxNum=@boxNum";
                cmd.Connection = con;
        ...
        private void loadDataIntoGridViewBoxes()
        {
            if (textBoxBoxNumber.Text != "")
            {
                try
                {
                    ...
                    cmd.Connection = con;
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("@docKind","نقدى");
                    SqlDataAdapter adapter = new SqlDataAdapter();
                    adapter.SelectCommand = cmd;
